...
async def main():
    ...
    print(f" Thse are sent just to confirm =  email={EMAIL}  nonce={NONCE} SERVER_PUBLIC_KEY={SERVER_PUBLIC_KEY.hex()}")
    community.ez_send(server, SubmissionPayload(EMAIL, GITHUB_URL, NONCE))

    for retry in range(12):          
        try:
            await asyncio.wait_for(community.done.wait(), timeout=10)
            break                 
